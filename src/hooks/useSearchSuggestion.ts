...
export default function useSearchSuggestion(params: {
    query: string;
    sessionToken: string;
    lngLat: LonLat;
}) {
    ...
    const { data: suggestionData, isLoading: loadingSearchSuggestion, error: errorSearchSuggestion } = useQuery({
        ...
        queryFn: () => fetchSearchSuggestion({ query: params.query, sessionToken: params.sessionToken, lngLat: params.lngLat }),
